...
if [[ $? -eq 0 ]]; then
    ...
    echo
    echo "1 # push changes of [$CURRENT_VERSION] to git repository"
    echo "git commit -am \"build new version\""
    echo "git push"
    echo
    echo "2 # publish new release by tagging new version [$CURRENT_VERSION] in git repository"
    echo "$SOURCE_URL/releases"
    echo ""
    echo "3 # push new version to Cocoapods trunk"
